...
if ($store.value.model === 'production') {
  ...
  useHead({
    ...
    title: '作者' + $route.params.name + '创作的文章 - ' + settings.site.title,
    meta: [
      {
        name: 'description',
        content: '作者' + $route.params.name + '创作的文章 - ' + settings.site.title
      }
    ]
  })
} else {
...
<template>
  <div class="flex flex-col">
    ...
    <div class="flex justify-center text-3xl p-2">
      <div class='flex'>
        <Icon name="account" width="36" height="36"/>
        {{ $route.params.name }}
      </div>
    </div>
    ...
    <div v-else>
      <div>暂时没有含有这个作者的文章欸，<nuxt-link to="/">返回首页</nuxt-link>看看别的。</div>
    </div>
  </div>
</template>
